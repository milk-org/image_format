...
)
{
    ...
    for(uint32_t kk=0; kk<zsize; kk++)
    {
        for(uint32_t jj1=0; jj1< ysize1; jj1++)
        {
            ...
            {
                data.image[IDimHDRc1].array.F[kk*xsize1*ysize1 + jj1*xsize1 + ii1] /= data.image[IDimHDRc1w].array.F[kk*xsize1*ysize1 + jj1*xsize1 + ii1];
            }
        }
    }
        ...
        for(int fiter = 0; fiter < NBfiter; fiter++)
        {
            ...
            for(uint32_t kk=0; kk<zsize; kk++)
            {
                for(uint32_t jj1=0; jj1< ysize1; jj1++)
                {
                    ...
                    {
                        pixline[ii1] = 0.3 * data.image[IDimHDRc1].array.F[kk*xsize1*ysize1 + jj1*xsize1 + ii1-1]
                ...
                for(uint32_t ii1=0; ii1< xsize1; ii1++)
                {
                    ...
                    {
                        pixcol[jj1] = 0.3 * data.image[IDimHDRc1].array.F[kk*xsize1*ysize1 + (jj1-1)*xsize1 + ii1]
                                      + 0.4 * data.image[IDimHDRc1].array.F[kk*xsize1*ysize1 + jj1*xsize1 + ii1]
...

    {
        ...
        for(int fiter = 0; fiter < NBfiter; fiter++)
        {
            ...
            for(uint32_t ii1=0; ii1< xsize1; ii1++)
            {
                for(uint32_t jj1=1; jj1< ysize1-1; jj1++)
                {
                    if(data.image[IDlayer].array.F[jj1*xsize1 + ii1] < data.image[IDlayermin].array.F[jj1*xsize1 + ii1])
                    {
                        data.image[IDlayer].array.F[jj1*xsize1 + ii1] = data.image[IDlayermin].array.F[jj1*xsize1 + ii1];
                    }
                }
            }
